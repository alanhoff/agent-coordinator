...
- 每个新部分都必须声明描述性证据，以及可重复执行的正向和反向证明命令。成功要求正向命令退出码为 `0`、反向命令退出码非零；失败则要求相反的组合。
...
- 工作流完成时会重新运行所有非豁免图记录的证明，包括已分解和已取代的历史节点，并要求每组证明都成功。所有要求和阻塞项也必须解决，且只能存在允许的终止状态。Coordinator 不会调用或检查版本控制系统。
...
持久化的 schema-v8 工作流文档位于 `~/.agent-coordinator/workflows`；v6 和 v7 文档会自动升级，其已有节点将永久获得证明豁免。以下命令均为只读，不会创建、锁定、修复、规范化、缓存或清理状态。
