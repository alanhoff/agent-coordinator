...
class PublicContractTests(unittest.TestCase):
    def test_schema_v7_refinement_and_node_split_public_contract(self) -> None:
    ...
    def test_surviving_source_and_metadata_are_complete(self) -> None:
        ...
        owners = {
            path.name
            for path in (RUNTIME / "coordinator").iterdir()
            if path.is_dir() and path.name != "__pycache__"
        }
        self.assertEqual(owners, {"cli", "routing", "state"})
        self.assertFalse((ROOT / "src").exists())
        self.assertEqual((ROOT / "skill" / "LICENSE").read_bytes(), (ROOT / "LICENSE").read_bytes())
        configuration = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
        self.assertNotIn("project", configuration)
        self.assertEqual(configuration["tool"]["ruff"]["target-version"], "py311")
        self.assertNotIn("VER" + "SION", (RUNTIME / "coordinator" / "__init__.py").read_text(encoding="utf-8"))
    ...
    def test_runtime_imports_only_standard_library_and_coordinator(self) -> None:
        ...
        for path in (RUNTIME / "coordinator").rglob("*.py"):
    ...
    def test_all_python_adapters_emit_json_for_invalid_invocations(self) -> None:
        ...
        for name in ("coordinator_state.py", "model_router.py"):
            result = subprocess.run(
                [sys.executable, "-I", str(ROOT / "skill" / "scripts" / name), "invalid", "--json"],
                text=True,
    ...
    def test_skill_directory_is_a_portable_installation_artifact(self) -> None:
        environment = {key: value for key, value in os.environ.items() if key != "PYTHONPATH"}
        environment["PYTHONDONTWRITEBYTECODE"] = "1"
        with tempfile.TemporaryDirectory() as temporary:
            destination = pathlib.Path(temporary) / "portable coordinator"
            shutil.copytree(ROOT / "skill", destination)
            for name in ("coordinator_state.py", "model_router.py"):
                result = subprocess.run(
                    [sys.executable, "-I", str(destination / "scripts" / name), "invalid", "--json"],
                    cwd=temporary,
                    text=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
    ...
    def test_self_contained_install_surface(self) -> None:
        ...
        for required in (
            ...
            "skill/",
            "complete installation artifact",
            "exact copy",
            "Keep role profiles inside `agents/roles/`",
        ):
            self.assertIn(required, instructions)
        self.assertNotIn("src/coordinator", instructions)
        skill_readme = (ROOT / "skill" / "README.md").read_text(encoding="utf-8")
        self.assertIn("complete Coordinator installation artifact", skill_readme)
        self.assertIn("scripts/lib/coordinator", skill_readme)
        self.assertEqual(
    ...
    def test_dynamic_runtime_public_contract(self) -> None:
        parser = build_state_parser()
        subparsers = next(
            action for action in parser._actions if isinstance(action, argparse._SubParsersAction)
        )
        ...
        self.assertTrue(commands.issubset(subparsers.choices))

        state = new_state({"path": "/repository", "identity": "0" * 64}, "task", "session")
        self.assertEqual(state["schema_version"], 7)
        self.assertEqual(
            set(state["runtime_graph"]),
            {"generation", "observations", "projections", "node_metadata", "gates", "loops", "adaptations"},
        )

        protocol = (ROOT / "skill" / "SKILL.md").read_text(encoding="utf-8")
        reference = (ROOT / "skill" / "references" / "dynamic-runtime-graphs.md").read_text(
            encoding="utf-8"
        )
        schema = (ROOT / "skill" / "references" / "state-schema.md").read_text(encoding="utf-8")
        combined = " ".join((protocol + "\n" + reference + "\n" + schema).split())
        ...
        for required in (
            "schema version 7",
            "physically acyclic",
            "arbitrary acyclic DAG",
            "versioned acyclic iterations",
            "complete configured panel",
            "progress cannot move backwards",
            "exactly two branches with a join",
            "fragment-local topological check",
            "runtime-generated replacement descendants remain blocked",
            "failed leaves remain observable",
            "128",
            "256",
        ):
            self.assertIn(required.casefold(), combined.casefold())
    ...
    def test_ci_uses_latest_platforms_with_only_minimum_python(self) -> None:
        ...
        self.assertIn("ruff check --no-cache skill/scripts tests", ci)
        self.assertIn("python -m compileall -q skill/scripts tests", ci)
        self.assertNotIn("ruff check --no-cache src", ci)
